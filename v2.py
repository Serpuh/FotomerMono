...
def rotate_points_around_x(points: torch.Tensor, angle_deg: float, center: Optional[torch.Tensor] = None) -> torch.Tensor:
    """
    Rotate points around X axis by angle_deg degrees.
    - points: tensor with last dimension 3, e.g. (B,H,W,3), (H,W,3), (N,3)
    - center: optional pivot with shape broadcastable to points[..., :3] (e.g. (3,) or (B,1,1,3))
    Returns a new tensor with rotated points.
    """
    if points.shape[-1] != 3:
        raise ValueError("Last dimension of points must be 3 (x,y,z).")
    ...
    R = torch.tensor([[1., 0., 0.],
                      [0., c, -s],
                      [0., s,  c]], device=device, dtype=dtype)  # (3,3)

    # preserve infinite (masked) points: rotate only finite points
    finite_mask = torch.isfinite(points).all(dim=-1, keepdim=True)  # shape broadcastable to last dim
    pts = points.clone()

    # Move to origin if center provided
    if center is not None:
        # Make center broadcastable: it should have last dim 3
        if center.shape[-1] != 3:
            raise ValueError("center must have last dimension 3")
        # broadcast subtraction
        pts = pts - center

    # Apply rotation: pts has shape (..., 3); do matrix multiply
    # Use einsum for clarity and broadcasting
    rotated = torch.einsum('...i,ij->...j', pts, R.T)

    if center is not None:
        rotated = rotated + center

    # keep infinite values as they were (so mask stays)
    rotated = torch.where(finite_mask, rotated, points)

    return rotated

def rotate_normals_around_x(normals: torch.Tensor, angle_deg: float) -> torch.Tensor:
    """
    Rotate normals (last dim 3) by angle_deg and re-normalize.
    """
    if normals.shape[-1] != 3:
        raise ValueError("Last dimension of normals must be 3.")
    ...
    finite_mask = torch.isfinite(normals).all(dim=-1, keepdim=True)
    rotated = torch.einsum('...i,ij->...j', normals, R.T)
    rotated = F.normalize(rotated, dim=-1, eps=1e-6)
    rotated = torch.where(finite_mask, rotated, normals)
    return rotated
...
class MoGeModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Union[str, Path, IO[bytes]], model_kwargs: Optional[Dict[str, Any]] = None, **hf_kwargs) -> 'MoGeModel':
        ...
        if Path(pretrained_model_name_or_path).exists():
            ...
            print("Cant find model "+pretrained_model_name_or_path)
            """
        ...
        #checkpoint_path = "C:\\MoGe\\Models\\moge-2-vitl-normal\\model.pt"
    ...
    def infer(
        ...
        with torch.autocast(device_type=self.device.type, dtype=torch.float32):
            ...
            ff = torch.rad2deg( torch.atan((aspect_ratio / (1 + aspect_ratio ** 2) ** 0.5)/focal) ) * 2.0
            ...
            # Filter by max_depth_m (in meters) if requested.
            # This runs after metric scaling so depth is in metric units.
            if self.max_depth_m is not None and depth is not None:
                md = torch.as_tensor(self.max_depth_m, device=depth.device, dtype=depth.dtype)
                # expand scalar to batch if necessary
                if md.ndim == 0:
                    md = md.expand(depth.shape[0])
                # valid depth mask: True where depth <= max_depth_m
                valid_depth_mask = depth <= md[:, None, None]
                if mask_binary is not None:
                    mask_binary &= valid_depth_mask
                else:
                    mask_binary = valid_depth_mask
                if points is not None:
            ...
            # Apply max_depth filter (remove points with Z > max_depth)
            """
            if max_depth is not None and depth is not None:
                max_d = torch.as_tensor(max_depth, device=depth.device, dtype=depth.dtype)
                # If max_d is scalar, compare directly; otherwise assume per-batch thresholds
                if max_d.ndim == 0:
                    mask_depth = depth <= max_d
                else:
                    # reshape to (B,1,1) to broadcast across H,W
                    mask_depth = depth <= max_d.view(-1, 1, 1)
                if mask_binary is not None:
                    mask_binary &= mask_depth
                else:
                    mask_binary = mask_depth
            """

            #axis_angle = np.array([180, 0.0, 0.0])  # magnitude = angle, direction x-axis
            ...
            R_x = torch.tensor([[1,      0,       0],
                              [0,  cos_t,  -sin_t],
                              [0,  sin_t,   cos_t]], dtype=points.dtype, device=points.device)
