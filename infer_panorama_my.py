...
@click.option('--threshold', type=float, default=0.01, help='Threshold for removing edges. Defaults to 0.03. Smaller value removes more edges. "inf" means no thresholding.')
...
def main(
    ...
    for image_path in (pbar := tqdm(image_paths, desc='Total images', disable=len(image_paths) <= 1)):
        ...
        if save_glb_ or save_ply_ or show:
            ...
            mesh_scale: float = 2
            # Scale mesh vertices if scaling factor is not 1.0
            if mesh_scale != 1.0:
            ...
            if mesh_radius is not None:
                ...
                valid_vertices = distances <= mesh_radius
                # Keep only valid vertices and their associated faces
                vertex_map = np.full(len(vertices), -1, dtype=int)
                vertex_map[valid_vertices] = np.arange(np.sum(valid_vertices))
        
                new_faces = []
                for face in faces:
                    if all(valid_vertices[v] for v in face):
                        new_faces.append(vertex_map[face])
        
                vertices = vertices[valid_vertices]
                vertex_colors = vertex_colors[valid_vertices]
                if vertex_uvs is not None:
                    vertex_uvs = vertex_uvs[valid_vertices]
                faces = np.array(new_faces)

            radius_limit = 3
            # Apply radius limit from Z axis if specified
            if radius_limit is not None:
                # Calculate distance from Z axis (x^2 + y^2)
                distances_from_z_axis = np.sqrt(vertices[:, 0]**2 + vertices[:, 1]**2)
                valid_vertex_mask = distances_from_z_axis <= radius_limit
                
                # Filter vertices and their associated data
                valid_indices = np.where(valid_vertex_mask)[0]
                vertices = vertices[valid_vertex_mask]
                vertex_colors = vertex_colors[valid_vertex_mask]
                vertex_uvs = vertex_uvs[valid_vertex_mask]
                
                # Create a mapping from old indices to new indices
                index_mapping = {old_idx: new_idx for new_idx, old_idx in enumerate(valid_indices)}
                
                # Filter faces - keep only faces where all three vertices are valid
                valid_faces_mask = np.all(np.isin(faces, valid_indices), axis=1)
                faces = faces[valid_faces_mask]
                
                # Remap face indices to match filtered vertices
                faces = np.array([[index_mapping[idx] for idx in face] for face in faces])
